...
def delete_multiple():
    ...
    try:
        ...
        logger.debug(f"Получены ID для удаления: {graduate_ids}")

        if not graduate_ids:
            flash('Не выбраны выпускники для удаления.', 'danger')
            return redirect(url_for('main.index'))

        # Преобразование ID в целые числа и проверка
        graduate_ids = [int(id) for id in graduate_ids]
        logger.debug(f"Преобразованные ID: {graduate_ids}")

        # Проверка существования записей
        graduates_to_delete = Graduate.query.filter(Graduate.id.in_(graduate_ids)).all()
        logger.debug(f"Найдено записей для удаления: {len(graduates_to_delete)}")
        ...
        for graduate in graduates_to_delete:
            db.session.delete(graduate)
        db.session.commit()
        logger.debug(f"Удалено записей: {len(graduates_to_delete)}")
        ...
        for grad in graduates:
            ...
            new_id += 1
        db.session.commit()
        logger.debug("ID переиндексированы")
        ...
        try:
            ...
            if file and allowed_file(file.filename, {'csv', 'xlsx', 'xls'}):
                ...
                if not all(header in df.columns for header in expected_headers):
                    flash(
                        'Неверный формат файла. Ожидаемые столбцы: ID, Имя, Группа, Год выпуска, Факультет, Биография, Теги.',
                        'danger')
                    return redirect(url_for('graduate.upload'))
                ...
                for index, row in df.iterrows():
                    try:
                        ...
                        faculty = str(row['Факультет']).strip() if pd.notna(row['Факультет']) else ''
                        ...
                        if not group:
                            ...
                            continue
                        if not faculty:
                            flash(f"Ошибка в строке {index + 2}: Факультет не может быть пустым.", 'danger')
                            continue
                        if not graduation_year:
                        ...
                        tags_input = str(row['Теги']).strip() if pd.notna(row['Теги']) else ''
                        tags = []
